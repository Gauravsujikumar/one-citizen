...
async function safeParseResponse(response, defaultErrorMsg) {
  ...
  if (isJson) {
    try {
      ...
      if (!response.ok) {
        throw new Error(defaultErrorMsg + ` (Status ${response.status}): ${data.error || data.message || 'Unknown error'}`);
      }
      return data;
    } catch (e) {
      if (e.message && e.message.startsWith(defaultErrorMsg)) {
        throw e;
      }
    }
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Wait 800ms, then transition to Phase 2 (Government of Telangana emblem)
  setTimeout(() => {
    ...
    // Wait another 600ms, then transition to the Login Screen (or check token and load dashboard)
    setTimeout(() => {
